...
async def get_age_markup(language, back_btn = None):
    age_buttons = []
    # Add button for -15
    age_buttons.append(InlineKeyboardButton("-14", callback_data=f"register_age_{language}_-15"))
    # Create buttons for ages 15 to 34
    for age in range(16, 35):
    ...
    if back_btn:
            ...
            age_buttons.append(InlineKeyboardButton("❌", callback_data="close_profile"))
...
async def gift_premium_msg(user_id, language):
    if language == "English":
        ...
        buttons = InlineKeyboardMarkup([
            [InlineKeyboardButton("For a day - $1.08", callback_data=f"frgt_{user_id}_1_day")],
            [InlineKeyboardButton("For three days - $2.15", callback_data=f"frgt_{user_id}_3_days")],
            [InlineKeyboardButton("For a week - $8.61", callback_data=f"frgt_{user_id}_1_week")],
            [InlineKeyboardButton("For a month - $12.98", callback_data=f"frgt_{user_id}_1_month")]
        ])
    elif language == "Russian":
        ...
        buttons = InlineKeyboardMarkup([
            [InlineKeyboardButton("На день - 81₽", callback_data=f"frgt_{user_id}_1_day")],
            [InlineKeyboardButton("На три дня - 162₽", callback_data=f"frgt_{user_id}_3_days")],
            [InlineKeyboardButton("На неделю - 648₽", callback_data=f"frgt_{user_id}_1_week")],
            [InlineKeyboardButton("На месяц - 974₽", callback_data=f"frgt_{user_id}_1_month")]
        ])
    elif language == "Azerbejani":
        ...
        buttons = InlineKeyboardMarkup([
            [InlineKeyboardButton("Bir gün üçün - 1,08 $", callback_data=f"frgt_{user_id}_1_day")],
            [InlineKeyboardButton("Üç gün üçün - 2,15 $", callback_data=f"frgt_{user_id}_3_days")],
            [InlineKeyboardButton("Bir həftə üçün - 8,61 $", callback_data=f"frgt_{user_id}_1_week")],
            [InlineKeyboardButton("Bir ay üçün - 12,98 $", callback_data=f"frgt_{user_id}_1_month")]
        ])
    else:
        caption = "Invalid language specified."
        buttons = InlineKeyboardMarkup([])
